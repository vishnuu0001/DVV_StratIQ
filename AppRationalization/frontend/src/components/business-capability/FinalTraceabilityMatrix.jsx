// ---------------------------------------------------------------------------
// Author: Vishnuu A
// Scope: AppRationalization — frontend/src/components/business-capability (FinalTraceabilityMatrix.jsx)
// Date: 2026-03-13
// ---------------------------------------------------------------------------
...
// Function: FinalTraceabilityMatrix
const FinalTraceabilityMatrix = () => {
  ...
  useEffect(() => {
    const cached = localStorage.getItem('cache_traceability');
    if (cached) {
      try {
        const p = JSON.parse(cached);
        setTraceabilityData(p.data || []);
        setSummary(p.summary || {});
        setLlmInsights(p.llm_insights || null);
      } catch (_) {}
    }
  }, []);
  ...
  const fetchTraceabilityMatrix = useCallback(async () => {
    try {
      ...
      if (response.data.status === 'success') {
        ...
        localStorage.setItem('cache_traceability', JSON.stringify({
          data: response.data.data || [],
          summary: response.data.summary || {},
          llm_insights: response.data.llm_insights || null,
        }));
        setError(null);
      } else {
  ...
  const handleClear = async () => {
    ...
    try {
      ...
      setSummary(null);
      setLlmInsights(null);
      localStorage.removeItem('cache_traceability');
      setError(null);
    } catch (err) {
  ...
  const normalizeAction = useCallback((action) => {
    const value = String(action || '').trim();
    if (!value) return '';
    if (value.toLowerCase().startsWith('migrate')) return 'Migrate to SAP';
    return value;
  }, []);

  const getAppKey = useCallback((item) => {
    return item.app_id || `${item.application || 'unknown-app'}::${item.repository || 'unknown-repo'}`;
  }, []);

  // Assign each app exactly one dominant action using highest-severity-wins.
  // This keeps Retain + Migrate + Decommission equal to total apps.
  const appCounts = useMemo(() => {
    const priority = { Decommission: 3, 'Migrate to SAP': 2, Retain: 1 };
    const allApps = new Set();
    const dominantByApp = {};

    traceabilityData.forEach((item) => {
      const appKey = getAppKey(item);
      const action = normalizeAction(item.action);
      allApps.add(appKey);

      const current = dominantByApp[appKey];
      if ((priority[action] ?? 0) > (priority[current] ?? 0)) {
        dominantByApp[appKey] = action;
      }
    });

    const bins = { Retain: 0, 'Migrate to SAP': 0, Decommission: 0 };
    Object.values(dominantByApp).forEach((action) => {
      if (bins[action] !== undefined) bins[action] += 1;
    });

    const summaryRetain = Number(summary?.applications_to_retain);
    const summaryMigrate = Number(summary?.applications_to_migrate);
    const summaryDecommission = Number(summary?.applications_to_decommission);
    const hasSummaryCounts = Number.isFinite(summaryRetain)
      && Number.isFinite(summaryMigrate)
      && Number.isFinite(summaryDecommission);

    return {
      total: allApps.size || summary?.total_applications || 0,
      Retain: hasSummaryCounts ? summaryRetain : bins.Retain,
      'Migrate to SAP': hasSummaryCounts ? summaryMigrate : bins['Migrate to SAP'],
      Decommission: hasSummaryCounts ? summaryDecommission : bins.Decommission,
      _dominantByApp: dominantByApp,
    };
  }, [traceabilityData, summary, normalizeAction, getAppKey]);

  const getDisplayAction = useCallback((item) => {
    return appCounts._dominantByApp[getAppKey(item)] || normalizeAction(item.action);
  }, [appCounts, getAppKey, normalizeAction]);

  // Filter by dominant app action so tab counts and row groups remain consistent.
  const filteredData = filterAction === 'All'
    ? traceabilityData
    : traceabilityData.filter(item => appCounts._dominantByApp[getAppKey(item)] === filterAction);

  // Function: getRedundancyColor
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {!loading && !error && (
          <div className="mb-6 flex flex-wrap gap-2">
            ...
            >
              All ({appCounts.total || summary?.total_applications || 0})
            </button>
            <button
            ...
            >
              Migrate ({appCounts['Migrate to SAP']})
            </button>
            <button
            ...
            >
              Decommission ({appCounts.Decommission})
            </button>
          </div>
        )}
                  ...
                  filteredData.map((item, index) => (
                    <React.Fragment key={index}>
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold ${getActionColor(getDisplayAction(item))}`}>
                            {getActionIcon(getDisplayAction(item))}
        ...
        {/* ── LLM Insights Panel ── */}
        {!loading && !error && llmInsights?.available && (
          <div className="mt-8 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl shadow-md p-8 border border-indigo-200">
            <div className="flex items-center gap-3 mb-6">
              <span className="text-3xl">🤖</span>
              <div>
                <h3 className="text-2xl font-bold text-indigo-900">AI-Powered Rationalization Insights</h3>
                <p className="text-sm text-indigo-600">Generated by LLM model: <strong>{modelDisplayName(llmInsights.model_used)}</strong></p>
              </div>
            </div>

            {llmInsights.rationalization_narrative && (
              <div className="mb-6 bg-white rounded-lg p-5 border border-indigo-100 shadow-sm">
                <h4 className="font-semibold text-gray-800 mb-2 flex items-center gap-2">
                  <span>📋</span> Rationalization Strategy
                </h4>
                <p className="text-gray-700 leading-relaxed">{llmInsights.rationalization_narrative}</p>
              </div>
            )}

            {llmInsights.priority_actions?.length > 0 && (
              <div className="mb-6">
                <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                  <span>✅</span> Priority Actions
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {llmInsights.priority_actions.map((action, i) => (
                    <div key={i} className="bg-white rounded-lg p-4 border border-indigo-100 shadow-sm">
                      <div className="flex items-start gap-3">
                        <span className="flex-shrink-0 w-7 h-7 rounded-full bg-indigo-600 text-white text-sm font-bold flex items-center justify-center">{action.rank || i + 1}</span>
                        <div>
                          <p className="font-semibold text-gray-800">{action.action}</p>
                          {action.apps_affected && <p className="text-sm text-indigo-600 mt-0.5">{action.apps_affected}</p>}
                          {action.rationale && <p className="text-sm text-gray-600 mt-1">{action.rationale}</p>}
                          {action.timeline && <p className="text-xs text-emerald-600 mt-1 font-medium">⏱ {action.timeline}</p>}
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            {llmInsights.migration_roadmap?.length > 0 && (
              <div className="mb-6">
                <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                  <span>🗺️</span> LLM Migration Roadmap
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {llmInsights.migration_roadmap.map((phase, i) => (
                    <div key={i} className="bg-white rounded-lg p-4 border-t-4 border-indigo-500 shadow-sm">
                      ...
                      {phase.description && <p className="text-sm text-gray-600 mb-2">{phase.description}</p>}
                      {phase.apps_count > 0 && (
                        <p className="text-xs font-medium text-indigo-700 bg-indigo-50 rounded px-2 py-1">{phase.apps_count} apps</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {llmInsights.decommission_rationale && (
              <div className="mb-6 bg-white rounded-lg p-5 border border-red-100 shadow-sm">
                <h4 className="font-semibold text-gray-800 mb-2 flex items-center gap-2">
                  <span>🗑️</span> Decommission Rationale
                </h4>
                <p className="text-gray-700 leading-relaxed">{llmInsights.decommission_rationale}</p>
              </div>
            )}

            {llmInsights.risk_assessment?.length > 0 && (
              <div>
                <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                  <span>⚠️</span> Risk Assessment
                </h4>
                <ul className="space-y-2">
                  {llmInsights.risk_assessment.map((risk, i) => (
