// ---------------------------------------------------------------------------
// Author: Vishnuu A
// Scope: AppRationalization — frontend/src/components/business-capability (BusinessCapabilityMapping.jsx)
// Date: 2025-12-24
// ---------------------------------------------------------------------------
...
// Function: BusinessCapabilityMapping
const BusinessCapabilityMapping = () => {
  ...
  const [analysisClearKey, setAnalysisClearKey] = useState(0);
  const [analysisRefreshKey, setAnalysisRefreshKey] = useState(0);
  const hasFetched = useRef(false);

  // Restore cached mapping data on mount
  useEffect(() => {
    const cached = localStorage.getItem('cache_capability_mapping');
    if (cached) {
      try {
        const p = JSON.parse(cached);
        setApplications(p.applications || []);
        setTotalPages(p.totalPages || 1);
        setTotalApplications(p.totalApplications || 0);
  ...
  const fetchCapabilityMapping = useCallback(async (page) => {
    try {
      ...
      if (response.data?.data) {
        ...
        localStorage.setItem('cache_capability_mapping', JSON.stringify({
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    if (viewMode === 'mapping') {
      await fetchCapabilityMapping(currentPage);
    } else if (viewMode === 'analysis') {
      // Clear frontend cache so CapabilityAnalysisView fetches fresh from backend
      localStorage.removeItem('cache_capability_analysis');
  ...
  const handleClear = async () => {
    ...
    try {
      ...
      setTotalPages(1);
      localStorage.removeItem('cache_capability_mapping');
      localStorage.removeItem('cache_capability_analysis');
      setAnalysisClearKey(k => k + 1);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {viewMode === 'analysis' && <CapabilityAnalysisView key={`${analysisClearKey}-${analysisRefreshKey}`} forceRefresh={analysisRefreshKey > 0} onViewDetails={handleCapabilityClick} />}
      </div>
    </div>
  );
};
...
// Function: CapabilityAnalysisView
const CapabilityAnalysisView = ({ onViewDetails, forceRefresh = false }) => {
  ...
  const fetchAnalysis = useCallback(async (skipCache = false) => {
    try {
      ...
      if (response.data?.data) {
        ...
        localStorage.setItem('cache_capability_analysis', JSON.stringify(response.data.data));
      }
    } catch (err) {
  ...
  useEffect(() => {
    if (forceRefresh) {
      // Show stale cache immediately while fetching fresh data in background
      const cached = localStorage.getItem('cache_capability_analysis');
      if (cached) {
        try { setAnalysis(JSON.parse(cached)); setLoading(false); } catch (_) {}
        fetchAnalysis(true); // background refresh
      } else {
        fetchAnalysis(false);
      }
    } else {
      const cached = localStorage.getItem('cache_capability_analysis');
      if (cached) {
        try { setAnalysis(JSON.parse(cached)); setLoading(false); return; } catch (_) {}
      }
      // No cache + no forceRefresh = data was just cleared; show empty state, wait for Generate
      setLoading(false);
    }
  }, [forceRefresh, fetchAnalysis]);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-gray-500 text-sm font-medium">Generating capability analysis…</p>
      </div>
    );
  }
  ...
  if (!analysis) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3 text-center">
        ...
        <p className="text-gray-500 font-medium">No analysis data. Click <span className="font-semibold text-green-600">Generate</span> to run capability analysis.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      {refreshing && (
        <div className="flex items-center gap-2 mb-4 px-4 py-2 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-700">
          <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/>
          </svg>
          Refreshing analysis in background…
        </div>
      )}
      {/* Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500">
          ...
          <p className="text-xs text-gray-500 mt-2">Unique apps eligible for elimination</p>
        </div>
      </div>
      ...
      {/* ── LLM Insights Panel ── */}
      {analysis?.llm_insights?.available && (
        <div className="mt-10 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl shadow-md p-8 border border-indigo-200">
          <div className="flex items-center gap-3 mb-6">
            <span className="text-3xl">🤖</span>
            <div>
              <h3 className="text-2xl font-bold text-indigo-900">AI-Powered Capability Insights</h3>
              <p className="text-sm text-indigo-600">Generated by LLM model: <strong>{modelDisplayName(analysis.llm_insights.model_used)}</strong></p>
            </div>
          </div>

          {analysis.llm_insights.portfolio_insights && (
            <div className="mb-6 bg-white rounded-lg p-5 border border-indigo-100 shadow-sm">
              <h4 className="font-semibold text-gray-800 mb-2 flex items-center gap-2">
                <span>📋</span> Portfolio Insights
              </h4>
              <p className="text-gray-700 leading-relaxed">{analysis.llm_insights.portfolio_insights}</p>
            </div>
          )}

          {analysis.llm_insights.capability_strategy && (
            <div className="mb-6 bg-white rounded-lg p-5 border border-indigo-100 shadow-sm">
              <h4 className="font-semibold text-gray-800 mb-2 flex items-center gap-2">
                <span>🎯</span> Capability Strategy
              </h4>
              <p className="text-gray-700 leading-relaxed">{analysis.llm_insights.capability_strategy}</p>
            </div>
          )}

          {analysis.llm_insights.consolidation_targets?.length > 0 && (
            <div className="mb-6">
              <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                <span>🔀</span> Consolidation Targets
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {analysis.llm_insights.consolidation_targets.map((target, i) => (
                  <div key={i} className="bg-white rounded-lg p-4 border border-indigo-100 shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      <p className="font-semibold text-gray-800">{target.capability}</p>
                      <span className={`text-xs px-2 py-1 rounded-full font-bold ${
                        target.recommended_action === 'Eliminate' ? 'bg-red-100 text-red-700' :
                        target.recommended_action === 'Consolidate' ? 'bg-orange-100 text-orange-700' :
                        'bg-green-100 text-green-700'
                      }`}>{target.recommended_action}</span>
                    </div>
                    {target.apps_affected > 0 && (
                      <p className="text-sm text-indigo-600 mb-1">{target.apps_affected} apps affected</p>
                    )}
          ...
          {analysis.llm_insights.modernization_priorities?.length > 0 && (
            <div className="mb-6">
              <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                <span>🚀</span> Modernization Priorities
              </h4>
              <div className="space-y-3">
                {analysis.llm_insights.modernization_priorities.map((item, i) => (
                  <div key={i} className="bg-white rounded-lg px-4 py-3 border border-indigo-100 shadow-sm flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-gray-800">{item.capability}</p>
                      <p className="text-sm text-gray-600 mt-0.5">{item.action}</p>
                      {item.business_impact && <p className="text-xs text-emerald-700 mt-1">{item.business_impact}</p>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {analysis.llm_insights.quick_wins?.length > 0 && (
            <div>
              <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                <span>⚡</span> Quick Wins (within 3 months)
              </h4>
              <ul className="space-y-2">
                {analysis.llm_insights.quick_wins.map((win, i) => (
                  <li key={i} className="flex items-start gap-2 bg-white rounded-lg px-4 py-3 border border-emerald-200 shadow-sm">
