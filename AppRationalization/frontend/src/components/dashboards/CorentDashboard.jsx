// ---------------------------------------------------------------------------
// Author: Vishnuu A
// Scope: AppRationalization — frontend/src/components/dashboards (CorentDashboard.jsx)
// Date: 2025-08-07
// ---------------------------------------------------------------------------
import React, { useState } from 'react';
import { Server, Package, Globe, Database } from 'lucide-react';

// Function: CorentDashboard
const CorentDashboard = ({ data }) => {
  ...
  // Count unique applications by app_id (deduplicated) to avoid double-counting
  // when IndustryData + CorentData both contribute rows for the same app
  const uniqueAppIds = new Set(
    serverAppMapping.map(a => a.app_id || a.app_name).filter(Boolean)
  );
  const totalApplicationsCount = data.total_applications ||
    (uniqueAppIds.size > 0 ? uniqueAppIds.size : serverAppMapping.length);
        ...
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-900 text-3xl font-bold mt-1">{totalApplicationsCount}</p>
            </div>
