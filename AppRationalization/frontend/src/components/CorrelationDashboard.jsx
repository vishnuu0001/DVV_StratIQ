// ---------------------------------------------------------------------------
// Author: Vishnuu A
// Scope: AppRationalization — frontend/src/components (CorrelationDashboard.jsx)
// Date: 2025-08-11
// ---------------------------------------------------------------------------
import React, { useState, useEffect } from 'react';
import { toast } from 'react-toastify';
import { getCorrelationData, startCorrelation, getCorrelationJobStatus, getOllamaStatus, getWorkspaceColumnUpdates, getLlmAnalysis, getWorkspaceRuns, rerunLlmAnalysis, getAppsByCloudGroup, getAppDetail, getWorkspaceCastRows, getWorkspaceCorentRows, getWorkspaceBizRows } from '../services/api';
...
const PIPELINE_STEPS = [
  { id: 'predict',     label: 'Predicting null values with AI (STRATIQ_InfraScan, STRATIQ_CodeAnalysis, Industry)' },
...
// ””€ Small helper: render each pipeline step ”””””””””””””””””””””””””””””””””””€
...
const PipelineStepList = ({ activeStep }) => (
  <ul className="mt-4 space-y-2 text-sm text-left max-w-lg mx-auto">
    {PIPELINE_STEPS.map((step, idx) => {
      const allDone = activeStep === 'all_done';
      const rawIdx  = allDone ? PIPELINE_STEPS.length : PIPELINE_STEPS.findIndex(s => s.id === activeStep);
      // When backend step doesn't map to a UI step yet, default to step 0 active
      const stepIdx = (!allDone && rawIdx === -1) ? 0 : rawIdx;
      const done   = allDone || idx < stepIdx;
      const active = !allDone && idx === stepIdx;
      return (
        <li key={step.id} className="flex items-center gap-2">
          ...
          <span className={done ? 'text-green-700 font-medium' : active ? 'text-blue-700 font-medium' : 'text-gray-400'}>
            {step.label}
          </span>
        </li>
      );
...
// Detect stale/placeholder analysis — check if all priorities are "..." or empty
const PLACEHOLDER_APP_IDS = new Set(['...', 'app_id', '<app_id>', 'actual_app_id', '']);
// Function: isPlaceholderAppId
const isPlaceholderAppId = (id) => !id || PLACEHOLDER_APP_IDS.has((id || '').toLowerCase().trim()) || id.startsWith('<') || id.startsWith('...');

const MODERNIZATION_ACTION_COLORS = {
...
// Function: PipelineStatsBanner
const PipelineStatsBanner = ({ pipelineStats, modelUsed }) => {
...
// Function: UnavailableAnalysisNotice
const UnavailableAnalysisNotice = ({ llmAnalysis, onRegenerate, rerunning }) => (
  <div className="bg-amber-50 border border-amber-200 rounded-xl p-6">
    ...
    {onRegenerate && (
      <button
        ...
        disabled={rerunning}
        className="mt-4 inline-flex items-center gap-2 px-5 py-2 rounded-lg bg-amber-600 text-white text-sm font-medium hover:bg-amber-700 disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
      >
        {rerunning ? <Loader size={15} className="animate-spin" /> : <Brain size={15} />}
        {rerunning ? 'Generating AI Analysis…' : 'Retry AI Analysis'}
      </button>
    )}
  </div>
);

// Function: StaleOrEmptyAnalysisNotice
const StaleOrEmptyAnalysisNotice = ({ pipelineStats, llmAnalysis, isStale, isEmpty, onRegenerate, rerunning }) => (
  <div className="space-y-4">
    {/* Pipeline stats banner still shown */}
    <PipelineStatsBanner pipelineStats={pipelineStats} modelUsed={llmAnalysis.model_used} />
    <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 text-center">
      <Brain size={32} className="text-blue-400 mx-auto mb-3" />
      <h3 className="font-semibold text-blue-800 mb-1">
        {isStale && !isEmpty ? 'AI Analysis Has Stale Data' : 'AI Analysis Content Not Available'}
      </h3>
      <p className="text-blue-600 text-sm mb-4">
        {isStale && !isEmpty
          ? 'The stored analysis was generated before app data was loaded and contains placeholder values. Click below to regenerate with real data.'
...
// Function: CloudReadinessSection
const CloudReadinessSection = ({ cloudReadiness }) => {
  if (!cloudReadiness) return null;
  return (
    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5">
      <h4 className="font-semibold text-emerald-800 mb-2">Cloud Readiness Insights</h4>
      <p className="text-emerald-700 text-sm leading-relaxed">{cloudReadiness}</p>
    </div>
  );
};

// Function: RiskObservationsCard
const RiskObservationsCard = ({ items }) => {
...
// Function: RiskAndRecommendationsSection
const RiskAndRecommendationsSection = ({ riskObservations, recommendations }) => (
  <div className="grid md:grid-cols-2 gap-4">
    <RiskObservationsCard items={riskObservations} />
    <RecommendationsCard items={recommendations} />
  </div>
);

// Function: CorrelationQualitySection
const CorrelationQualitySection = ({ correlationQuality }) => {
  if (!correlationQuality) return null;
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-xl p-5">
      <h4 className="font-semibold text-gray-700 mb-2">Data Quality Assessment</h4>
      <p className="text-gray-600 text-sm">{correlationQuality}</p>
    </div>
  );
};

// Function: TechnicalDebtSection
const TechnicalDebtSection = ({ summary }) => {
  if (!summary) return null;
  return (
    <div className="bg-orange-50 border border-orange-200 rounded-xl p-5">
      <h4 className="font-semibold text-orange-800 mb-2">Technical Debt Summary</h4>
      <p className="text-orange-700 text-sm leading-relaxed">{summary}</p>
    </div>
  );
};

// Function: MigrationRoadmapPhase
const MigrationRoadmapPhase = ({ phase, index, onDrillDown }) => {
...
// Function: MigrationRoadmapSection
const MigrationRoadmapSection = ({ roadmap, onDrillDown }) => {
  ...
  return (
    <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-5">
      ...
      <div className="space-y-3">
        {roadmap.map((phase, i) => (
          <MigrationRoadmapPhase key={i} phase={phase} index={i} onDrillDown={onDrillDown} />
        ))}
      </div>
    </div>
  );
...
// Function: ModernizationPrioritiesSection
const ModernizationPrioritiesSection = ({ llmAnalysis, onDrillDown }) => {
  const [prioSearch, setPrioSearch] = useState('');
  const [prioPage,   setPrioPage]   = useState(1);

  const priorities = llmAnalysis.modernization_priorities || [];
  if (!(priorities.length > 0)) return null;

  const term = prioSearch.toLowerCase();
  const filtered = priorities.filter(item =>
    // Filter out placeholder entries
    item.app_id && !isPlaceholderAppId(item.app_id) && (
    !term ||
    ...
    <div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm border-collapse">
          ...
          <tbody>
            {visible.map((item, i) => {
              const actionColor = MODERNIZATION_ACTION_COLORS[item.recommended_action] ?? 'bg-gray-100 text-gray-700';
              return (
                <tr
                  key={item.app_id || i}
...
// Function: PerAppNotesSection
const PerAppNotesSection = ({ notes, onDrillDown }) => {
...
const LLMAnalysisPanel = ({ llmAnalysis, pipelineStats, onRegenerate, rerunning, onDrillDown }) => {
  ...
  if (!llmAnalysis.available) {
    return <UnavailableAnalysisNotice llmAnalysis={llmAnalysis} onRegenerate={onRegenerate} rerunning={rerunning} />;
  }

  // Detect stale/placeholder analysis — check if all priorities are "..." or empty
  const priorities = llmAnalysis.modernization_priorities || [];
  const isStale = priorities.length === 0 || priorities.every(p => isPlaceholderAppId(p.app_id));

  // Analysis record exists but content is empty or stale — offer regeneration
  const isEmpty = !llmAnalysis.summary && !llmAnalysis.executive_summary;
  if (isEmpty || isStale) {
    return (
      <StaleOrEmptyAnalysisNotice
  ...
  return (
    <div className="space-y-6">
      ...
      <CloudReadinessSection cloudReadiness={llmAnalysis.cloud_readiness} />

      <RiskAndRecommendationsSection
        riskObservations={llmAnalysis.risk_observations}
        recommendations={llmAnalysis.recommendations}
      />

      <CorrelationQualitySection correlationQuality={llmAnalysis.correlation_quality} />

      <TechnicalDebtSection summary={llmAnalysis.technical_debt_summary} />

      <MigrationRoadmapSection roadmap={llmAnalysis.migration_roadmap} onDrillDown={onDrillDown} />

      <ModernizationPrioritiesSection llmAnalysis={llmAnalysis} onDrillDown={onDrillDown} />

      <PerAppNotesSection notes={llmAnalysis.per_app_notes} onDrillDown={onDrillDown} />
    </div>
  );
};
...
// Function: WorkspacePredictionPanel
...
// Function: restoreCorrelationData
const restoreCorrelationData = async (setCorrelationData, setActiveTab) => {
  try {
...
// Function: applyLlmAnalysis
const applyLlmAnalysis = (a, setLlmAnalysis, setPipelineStats) => {
  setLlmAnalysis(a);
...
// Function: restoreLlmAnalysis
// Restore LLM analysis from DB (independent of whether correlation loaded)
// Function: restoreLlmAnalysis
const restoreLlmAnalysis = async (setLlmAnalysis, setPipelineStats) => {
  try {
    const llmRes = await getLlmAnalysis();
    if (llmRes.data.status === 'success') {
      applyLlmAnalysis(llmRes.data.analysis, setLlmAnalysis, setPipelineStats);
    } else if (llmRes.data.status === 'stale' && llmRes.data.analysis) {
      // Analysis exists but has placeholder data — load whatever we have and let
      // the user decide to regenerate manually via the Re-run button.
      applyLlmAnalysis(llmRes.data.analysis, setLlmAnalysis, setPipelineStats);
    }
  } catch {
...
// Function: restoreWorkspaceColumnUpdates
const restoreWorkspaceColumnUpdates = async (runId, setColumnUpdates) => {
  try {
    const colRes = await getWorkspaceColumnUpdates(runId);
    setColumnUpdates(colRes.data?.updates ?? []);
  } catch { /* non-critical */ }
};

// Function: restoreUpdatedRowsData
const restoreUpdatedRowsData = async (runId, setUpdatedRowsData) => {
  try {
    const [castRes, corentRes, bizRes] = await Promise.all([
      getWorkspaceCastRows(runId),
      getWorkspaceCorentRows(runId),
      getWorkspaceBizRows(runId),
    ]);
    setUpdatedRowsData({
...
// Function: restoreWorkspaceStats
// Restore workspace stats (for the Workspace tab and apps_with_ai_fill)
// Function: restoreWorkspaceStats
const restoreWorkspaceStats = async (setWorkspaceData, setPipelineStats, setColumnUpdates, setUpdatedRowsData) => {
  try {
    ...
    // Fetch column traceability and per-row updated_rows summary for the restored run
    await restoreWorkspaceColumnUpdates(latestRun.id, setColumnUpdates);
    await restoreUpdatedRowsData(latestRun.id, setUpdatedRowsData);
  } catch {
...
const CorrelationDashboard = () => {
  ...
  // Function: fetchLatestCorrelation
  const fetchLatestCorrelation = async () => {
    setLoading(true);
    await restoreCorrelationData(setCorrelationData, setActiveTab);
    await restoreLlmAnalysis(setLlmAnalysis, setPipelineStats);
    await restoreWorkspaceStats(setWorkspaceData, setPipelineStats, setColumnUpdates, setUpdatedRowsData);
    setLoading(false);
  };

  // Function: handleStartCorrelation
  const handleStartCorrelation = async () => {
    setCorrelating(true);
    setCorrelationStep('predict');  // 'copy' (workspace load) is part of the predict step

    // Map backend step names → UI PIPELINE_STEPS keys
    // 'copy' is the workspace-load phase — map it to 'predict' (the first visible step)
    const STEP_MAP = { copy: 'predict', correlate: 'correlate', predict: 'predict', consolidate: 'consolidate', analyse: 'analyse' };

    // Shared handler for the final result payload (works for both sync and async paths)
    // Function: handleResult
    const handleResult = async (data) => {
      // Flash all steps as completed before clearing
      setCorrelationStep('all_done');
      const { summary, llm_analysis, pipeline, workspace } = data;
      ...
      if (workspace?.run_id) {
        try {
          ...
          setColumnUpdates(colRes.data?.updates ?? []);
        } catch (e) { console.warn('Could not fetch workspace column updates:', e); }
        try {
          const [castRes, corentRes, bizRes] = await Promise.all([
        ...
        } catch (e) { console.warn('Could not fetch updated rows data:', e); }
      }
      ...
      // Clear the all_done banner after 4 seconds
      setTimeout(() => setCorrelationStep(null), 4000);
    };

    try {
      const startRes = await startCorrelation();
      const resData = startRes.data;

      // ── Path A: Old synchronous backend — returns full result immediately ──
      if (resData?.status === 'success') {
        await handleResult(resData);
        return;
      }

      // ── Path B: New async backend — returns job_id, poll for completion ──
      const jobId = resData?.job_id;
      if (!jobId) {
        throw new Error(resData?.message || 'Failed to start correlation job');
      }

      await new Promise((resolve, reject) => {
        const interval = setInterval(async () => {
          try {
            const pollRes = await getCorrelationJobStatus();
            const { status, step, result, error } = pollRes.data;

            if (STEP_MAP[step]) setCorrelationStep(STEP_MAP[step]);
            ...
            if (status === 'done') {
              ...
              clearInterval(interval);
              reject(new Error(error || 'Correlation pipeline failed'));
            }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-gray-100">
      ...
      <div className="bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          {/* Pipeline progress (visible while correlating or briefly after all done) */}
          {(correlating || correlationStep === 'all_done') && (
            <div className={`mt-6 p-5 rounded-xl text-center border ${
              correlationStep === 'all_done'
                ? 'bg-emerald-50 border-emerald-200'
                : 'bg-blue-50 border-blue-200'
            }`}>
              <p className={`font-semibold text-sm mb-2 ${
                correlationStep === 'all_done' ? 'text-emerald-700' : 'text-blue-800'
              }`}>
                {correlationStep === 'all_done'
                  ? '✓ AI pipeline completed successfully!'
                  : 'Running full AI pipeline. This may take a minute…'}
              </p>
        ...
        {!correlationData ? (
          <div className="bg-white rounded-xl border border-gray-200 p-16 text-center">
            ...
            <p className="text-gray-500 text-sm max-w-xl mx-auto">
              The pipeline will: predict missing values with AI †’ build a consolidated DB
              †’ match applications by APP ID (with name-based fallback) †’ generate an
            ...
            <div className="bg-white rounded-t-xl border-b border-gray-200">
              <div className="flex overflow-x-auto">
                {[
                  ...
                  { id: 'corent', label: 'STRATIQ_INFRA Dashboard' },
                  { id: 'cast', label: 'STRATIQ_CODEAnalysis Dashboard' },
