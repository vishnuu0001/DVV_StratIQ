...
from app.services.technical_assessment_service import (
    clear_dataset, import_business_validations, import_wave_inputs, latest_import,
)

technical_assessment_bp = Blueprint(
    "technical_assessment", __name__, url_prefix="/api/technical-assessment"
)
DATASET_KINDS = {"business-validations", "wave-inputs"}
...
def _import(kind, path, filename):
    try:
        importer = import_business_validations if kind == "business-validations" else import_wave_inputs
...
def list_rows(kind):
    if kind not in DATASET_KINDS:
        return jsonify({"error": "Unsupported dataset"}), 404
    import_record = latest_import("business_validations" if kind == "business-validations" else "wave_inputs")
    if not import_record:
        return jsonify({"items": [], "total": 0, "import": None})
    model = BusinessValidation if kind == "business-validations" else WaveInput
...
def clear(kind):
    if kind not in DATASET_KINDS:
        return jsonify({"error": "Unsupported dataset"}), 404
    try:
        dataset_type = "business_validations" if kind == "business-validations" else "wave_inputs"
