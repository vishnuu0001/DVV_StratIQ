# ---------------------------------------------------------------------------
# Author: Vishnuu A
# Scope: AppRationalization — backend/app/routes (upload_bp.py)
# Date: 2026-07-03
# ---------------------------------------------------------------------------
...
def get_upload_folder():
    default_upload = os.path.join(tempfile.gettempdir(), 'app_rationalization_uploads')
    upload_folder = current_app.config.get('UPLOAD_FOLDER', default_upload)
...
# Function: get_file_record_by_id
...
# Function: upload_infrastructure
...
def get_uploaded_files():
    ...
    for inf in infra_files:
        files.append({
            ...
            'size_kb': 1024,  # TODO: Calculate actual file size from storage backend
            'status': 'Ready'
        })
    ...
    for code in code_files:
        files.append({
            ...
            'size_kb': 2048,  # TODO: Calculate actual file size from storage backend
            'status': 'Ready'
        })
...
# Function: _to_preview_records
def _to_preview_records(dataframe, max_rows):
...
# Function: _find_best_header_row
def _find_best_header_row(raw_dataframe, scan_rows):
    """Choose the most likely header row from the first `scan_rows` rows by max non-empty cells."""
    header_index = 0
    max_non_empty = -1
...
# Function: _normalize_header_values
def _normalize_header_values(header_values):
    normalized_headers = []
...
# Function: _fallback_sheet_preview
def _fallback_sheet_preview(file_path, sheet_name, max_rows):
    ...
    header_index = _find_best_header_row(raw_dataframe, min(len(raw_dataframe), 10))
    normalized_headers = _normalize_header_values(raw_dataframe.iloc[header_index].tolist())
    ...
    return _sanitize_json_value(data_rows.to_dict(orient='records'))


# Function: _preview_csv_file
def _preview_csv_file(file_path, file_id, filename, max_rows):
    dataframe = pd.read_csv(file_path)
    preview_rows = _to_preview_records(dataframe, max_rows)
...
# Function: _preview_excel_file
def _preview_excel_file(file_path, file_id, filename, max_rows):
    ...
    for sheet_name, dataframe in workbook.items():
        ...
        if not preview_rows:
            preview_rows = _fallback_sheet_preview(file_path, sheet_name, max_rows)
...
# Function: preview_uploaded_file
...
def preview_uploaded_file(file_id):
    ...
    try:
        ...
        if file_ext == '.csv':
            return _preview_csv_file(file_path, file_id, filename, max_rows)

        if file_ext in ['.xlsx', '.xls', '.xlsm']:
            return _preview_excel_file(file_path, file_id, filename, max_rows)
...
# Function: _delete_file_record_and_disk_file
def _delete_file_record_and_disk_file(record):
    """Delete associated PDFReport rows and the on-disk file for a DB record, then delete the record."""
    pdf_reports_deleted = 0

    if record.file_path:
        pdf_reports_deleted = PDFReport.query.filter_by(source_pdf=record.file_path).delete()
        db.session.commit()
        print(f"[Delete] Deleted {pdf_reports_deleted} PDFReport entries for {record.file_path}")

    if record.file_path and os.path.exists(record.file_path):
        try:
            os.remove(record.file_path)
            print(f"[Delete] Removed file from disk: {record.file_path}")
        except Exception as e:
            print(f"Warning: Could not delete file {record.file_path}: {str(e)}")

    db.session.delete(record)
    db.session.commit()
    return pdf_reports_deleted
...
def delete_file(file_id):
    ...
    infra = Infrastructure.query.filter_by(file_id=file_id).first()
    if infra:
        pdf_reports_deleted = _delete_file_record_and_disk_file(infra)
        return jsonify({
            'success': True,
    ...
    code = CodeRepository.query.filter_by(file_id=file_id).first()
    if code:
        pdf_reports_deleted = _delete_file_record_and_disk_file(code)
        return jsonify({
            'success': True,
...
# Function: _store_infra_discovery_rows
def _store_infra_discovery_rows(rows, infra):
    """Bulk-insert/update InfrastructureDiscovery rows; return (extracted_count, updated_count)."""
    new_apps = []
    updated_count = 0
    for row_data in rows:
        ...
        if existing:
            ...
            new_apps.append(InfrastructureDiscovery(
    ...
    try:
        ...
        extracted_count = 0
    return extracted_count, updated_count


# Function: _handle_cast_excel_upload
def _handle_cast_excel_upload(cast_analysis, file_id, filename, file_path):
    """Process Excel/CSV CAST upload: populate CASTData and return jsonify response."""
    cast_records = CASTDataService.populate_from_excel_file(file_path)
    cast_analysis.extracted_at = datetime.utcnow()
    db.session.commit()
    return jsonify({
        'success': True,
        'message': 'Excel/CSV file processed successfully. CAST data loaded.',
        'file_type': 'excel',
        'sections': {
            ...
            'application_classification': {'label': 'Application Classification', 'count': 0, 'data': [], 'total': 0},
            'internal_architecture': {'label': 'Internal Architecture', 'count': 0, 'data': [], 'total': 0},
            'high_risk_applications': {'label': 'High-Risk Applications', 'count': 0, 'data': [], 'total': 0},
        },
        'extracted_rows': {
...
def extract_pdf_data(file_id):
    ...
    try:
        ...
        if report_type == 'infrastructure':
            ...
            extracted_count, updated_count = _store_infra_discovery_rows(rows, infra)
            ...
            return jsonify({
                'success': True,
                'message': 'Infrastructure & Network Discovery table extraction completed',
                'extracted_rows': {
...
# Function: _upsert_cast_analysis_rows
def _upsert_cast_analysis_rows(rows, cast_analysis, model_class, log_label):
    """Upsert rows into a CAST analysis table keyed by (cast_analysis_id, app_id)."""
    if not rows:
        return
    logger.info(f"[CAST Extract] Storing {len(rows)} {log_label} rows")
    new_objs = []
    for row_data in rows:
        ...
        if existing:
            ...
            new_objs.append(model_class(cast_analysis_id=cast_analysis.id, **row_data))
    if new_objs:
        db.session.bulk_save_objects(new_objs, return_defaults=False)
    db.session.commit()


# Function: _insert_cast_analysis_rows
def _insert_cast_analysis_rows(rows, cast_analysis, model_class, log_label):
    """Bulk insert rows into a CAST analysis table (no unique constraint — always inserts)."""
    if not rows:
        return
    logger.info(f"[CAST Extract] Storing {len(rows)} {log_label} rows")
    new_objs = [model_class(cast_analysis_id=cast_analysis.id, **row_data) for row_data in rows]
    if new_objs:
        db.session.bulk_save_objects(new_objs, return_defaults=False)
    db.session.commit()


# Function: extract_cast_analysis_data
    ...
    try:
        ...
        if file_ext in ['.xlsx', '.xls', '.csv', '.xlsm']:
            ...
            if not cast_analysis:
                cast_analysis = CASTAnalysis(file_id=file_id, filename=filename, file_path=file_path)
                db.session.add(cast_analysis)
                db.session.commit()
            return _handle_cast_excel_upload(cast_analysis, file_id, filename, file_path)
        ...
        _upsert_cast_analysis_rows(inventory_rows, cast_analysis, ApplicationInventory, 'Application Inventory')
        
        # Store Application Classification
        classification_rows = extraction_result['app_classification']
        _upsert_cast_analysis_rows(classification_rows, cast_analysis, ApplicationClassification, 'Application Classification')
        
        # Store Internal Architecture
        architecture_rows = extraction_result['internal_architecture']
        _insert_cast_analysis_rows(architecture_rows, cast_analysis, InternalArchitecture, 'Internal Architecture')
        
        # Store High-Risk Applications
        high_risk_rows = extraction_result.get('high_risk_applications', [])
        _insert_cast_analysis_rows(high_risk_rows, cast_analysis, HighRiskApplication, 'High-Risk Applications')
        ...
        logger.info("[CAST Extract] Extraction complete")
...
# Function: get_discovered_applications
...
# Function: import_business_capabilities
...
# Function: get_business_capabilities
...
# Function: get_business_capability
...
# Function: search_business_capabilities
...
# Function: upload_industry_templates
...
# Function: get_industry_templates
...
# Function: delete_industry_template
...
# Function: preview_industry_template
