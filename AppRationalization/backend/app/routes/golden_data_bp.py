...
# Function: _upsert_golden_data_item
def _upsert_golden_data_item(item):
    """Upsert one GoldenData record from an item dict; return 'created', 'updated', or None (skipped)."""
...
def bulk_upsert():
    ...
    created = updated = 0
    for item in records:
        status = _upsert_golden_data_item(item)
        if status == 'created':
            created += 1
        elif status == 'updated':
            updated += 1
