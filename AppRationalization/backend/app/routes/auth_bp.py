# ---------------------------------------------------------------------------
# Author: Vishnuu A
# Scope: AppRationalization — backend/app/routes (auth_bp.py)
# Date: 2025-08-14
# ---------------------------------------------------------------------------
import os
...
import requests
from flask import Blueprint, current_app, jsonify, redirect, request, g
...
def get_apps():
    return jsonify(
        {
            "applications": [
                {"key": APP_RATIONALIZATION, "name": "App Rationalization", "slug": "app-rationalization"},
                {"key": CODE_ANALYSIS, "name": "Code Analysis", "slug": "code-analysis"},
                {"key": INFRA_SCAN, "name": "Infra Rationalization", "slug": "infra-rationalization"},
                {"key": MODERNIZATION, "name": "Modernization Studio", "slug": "modernization"},
                {"key": NOVASTRA_ITSM, "name": "Novastra-ITSM", "slug": "novastra-itsm"},
                {"key": DASHBOARD, "name": "Dashboard", "slug": "dashboard"},
                {"key": SSDLC_PROCESS_ASSESSMENT, "name": "SSDLC Process Assessment", "slug": "ssdlc-process-assessment"},
                {"key": LAB_ROBOT, "name": "Lab Robot", "slug": "lab-robot"},
                {"key": OPPORTUNITY_TRACKER, "name": "Opportunity Tracker", "slug": "opportunity-tracker"},
                {
                    "key": AI_REMAN_CORE,
                    "name": "AI Reman Core",
                    "slug": "ai-reman-core",
                },
                {"key": AI_VEHICLE_LOAN, "name": "AI Vehicle Loan", "slug": "ai-vehicle-loan"},
                {"key": MICROSITE_DATA_ANALYSIS, "name": "Data Analysis Studio", "slug": "microsite-data-analysis"},
                {
                    "key": SUPPLY_CHAIN_DISRUPTION_MANAGER,
                    "name": "Supply Chain Disruption Manager",
                    "slug": "supply-chain-disruption-manager",
                },
                {"key": TRACEFORGE, "name": "TraceForge", "slug": "traceforge"},
            ]
        }
    )


# Function: oauth_provider_status
...
def login():
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error("Login database error: %s", str(exc))
        return jsonify({"error": "Authentication database is unavailable. Check backend database configuration."}), 503
    except Exception:  # noqa: BLE001 — login failures must remain JSON, never a Waitress plaintext 500
        db.session.rollback()
        current_app.logger.exception("Unexpected login error")
        return jsonify({"error": "Authentication failed because of an internal server error."}), 500
...
# Function: _fetch_github_user_data
def _fetch_github_user_data(code):
    """Exchange OAuth code for GitHub user info; returns (user_data, email, access_token)."""
    token_resp = requests.post(
    ...
    if not access_token:
        raise ValueError("github_access_token_missing")

    gh_headers = {
        "Accept": "application/vnd.github+json",
        "Authorization": f"Bearer {access_token}",
    }
    gh_user = requests.get("https://api.github.com/user", headers=gh_headers, timeout=20)
    gh_user.raise_for_status()
    user_data = gh_user.json()

    gh_emails = requests.get("https://api.github.com/user/emails", headers=gh_headers, timeout=20)
    email = None
    if gh_emails.ok:
    ...
    return user_data, email, access_token
...
def github_callback():
    ...
    try:
        user_data, email, _access_token = _fetch_github_user_data(code)
...
def delete_user(user_id):
    ...
    try:
        AuthService.delete_user(user_to_delete=user, acting_user=g.current_user)
        return jsonify({"success": True}), 200
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
