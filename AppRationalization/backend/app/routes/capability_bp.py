# ---------------------------------------------------------------------------
# Author: Vishnuu A
# Scope: Business Capability Routes
# Date: 2026-03-26
# ---------------------------------------------------------------------------
"""
...
# Function: get_capability_mapping
...
# Function: _build_capability_analysis_for_apps
def _build_capability_analysis_for_apps(applications):
    """Build an analysis dict for a list of app dicts sharing a capability."""
    if len(applications) > 1:
        tech_stacks = {}
        for app_dict in applications:
            tech = app_dict['technology_stack']
            tech_stacks.setdefault(tech, []).append(app_dict['app_name'])
        return {
...
def get_capability_details(capability_name):
    ...
    try:
        ...
        # Build analysis
        analysis = _build_capability_analysis_for_apps(applications)
...
# Function: debug_capability_details
...
def debug_capability_details(capability_name):
    ...
    try:
        ...
        current_app.logger.exception("Capability mapping lookup failed")
        return jsonify({
            'status': 'error',
            'message': str(e),
        }), 500
...
# Function: clear_capability_data
...
def clear_capability_data():
    ...
    try:
        ...
        # Invalidate in-memory and file-based LLM caches so next Generate fetches fresh
        try:
            import os
            from app.services.business_capability_service import BusinessCapabilityService, _CACHE_FILE
            BusinessCapabilityService._llm_insights_cache = {}
...
# Function: export_capability_mapping
