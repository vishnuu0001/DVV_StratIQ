# ---------------------------------------------------------------------------
# Author: Vishnuu A
# Scope: Orchestrates the three-step pipeline that runs when the user clicks
# Date: 2025-12-12
# ---------------------------------------------------------------------------
"""
...
# Function: _sample_non_null_records
...
# Function: _find_corent_for_app
def _find_corent_for_app(
    cast_r: Dict[str, Any],
    industry_r: Dict[str, Any],
    corent_by_server_name: Dict[str, Dict],
    corent_indexed: Dict[int, Dict],
    idx: int,
) -> Dict[str, Any]:
    """3-level corent record lookup: CAST server_name → Industry host → positional."""
...
# Function: _collect_ai_tracking
def _collect_ai_tracking(
    prefix_source_pairs: List[Tuple[str, Dict[str, Any]]],
) -> Tuple[List[str], Dict[str, float]]:
    """Collect namespaced ai_predicted columns and confidences from source records."""
    ai_predicted: List[str] = []
    ai_confidence: Dict[str, float] = {}
    for prefix, source_r in prefix_source_pairs:
    ...
    return ai_predicted, ai_confidence
...
class ConsolidatedDataService:
    ...
    # Function: predict_nulls_for_source
    @staticmethod
    ...
    # Function: build_consolidated_records
    @staticmethod
    def build_consolidated_records(
        ...
        for idx, app_id in enumerate(sorted(all_app_ids)):
            ...
            corent_r = _find_corent_for_app(
                cast_r, industry_r, corent_by_server_name, corent_indexed, idx
            )
            ...
            ai_predicted, ai_confidence = _collect_ai_tracking([
                ("industry", industry_r),
                ("cast", cast_r),
                ("corent", corent_r),
            ])
    ...
    # ------------------------------------------------------------------
    # Private LLM analysis helpers
    # ------------------------------------------------------------------

    # Function: _is_stale_llm_analysis
    @staticmethod
    def _is_stale_llm_analysis(analysis: Dict) -> bool:
        """Return True if the analysis dict contains only placeholder data."""
        prios = analysis.get("modernization_priorities") or []
        if not prios:
            return True
        _PLACEHOLDER_IDS = {"...", "app_id", "<app_id>", "actual_app_id", ""}
        return all(
            str(p.get("app_id", "")).strip().lower() in _PLACEHOLDER_IDS
            or str(p.get("app_id", "")).startswith("<")
            or str(p.get("app_id", "")).startswith("...")
            for p in prios
        )

    # Function: _run_llm_analysis_with_timeout
    @staticmethod
    def _run_llm_analysis_with_timeout(
        consolidated: List[Dict[str, Any]],
        stats: Dict[str, Any],
        predictions_summary: Dict[str, Any],
        timeout: int,
    ) -> Dict[str, Any]:
        """Run deep LLM correlation analysis in a thread with a hard timeout."""
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as _ex:
            ...
            try:
                ...
                return {
                    "available": False,
                    "summary": (
                        f"AI analysis timed out after {timeout}s. "
    ...
    def _execute_llm_step(
        ...
        stats: Dict[str, Any],
        predictions_summary: Dict[str, Any],
        correlation_result_id: Optional[int],
        timeout: int,
        LLMCorrelationAnalysis: Any,
    ) -> Tuple[Dict[str, Any], Optional[int]]:
        """Check LLM cache, run fresh analysis if stale, persist result; return (analysis, id)."""
        _CACHE_WINDOW_SECONDS = 7200
        recent_analysis: Optional[Any] = (
        ...
        if recent_analysis and recent_analysis.full_analysis:
            age = (datetime.datetime.utcnow() - recent_analysis.created_at).total_seconds()
            if age < _CACHE_WINDOW_SECONDS and not cls._is_stale_llm_analysis(recent_analysis.full_analysis):
                llm_analysis = recent_analysis.full_analysis
                llm_analysis_id = recent_analysis.id
                _use_cached = True
                logger.info(
                    "Returning cached LLM analysis (id=%d, age=%.0fs < %ds)",
                    recent_analysis.id, age, _CACHE_WINDOW_SECONDS,
                )
            elif cls._is_stale_llm_analysis(recent_analysis.full_analysis):
                logger.info(
                    "Cached LLM analysis (id=%d) has placeholder data — forcing fresh run",
                    recent_analysis.id,
                )

        if not _use_cached:
            llm_analysis = cls._run_llm_analysis_with_timeout(
                consolidated, stats, predictions_summary, timeout
            )

        if not _use_cached and llm_analysis.get("available") and correlation_result_id is not None:
            analysis_row = LLMCorrelationAnalysis(
                ...
                total_predictions_used   = stats.get("total_predictions", 0),
        ...
        return llm_analysis, llm_analysis_id

    # Function: _mark_pre_enriched
    @staticmethod
    def _mark_pre_enriched(records: List[Dict[str, Any]], ollama_available: bool) -> None:
        """ExcelPredictionService already enriched these records; just tag the AI-prediction bookkeeping fields."""
        for r in records:
    ...
    # Function: _load_consolidated_fallback
    @staticmethod
    def _load_consolidated_fallback() -> List[Dict[str, Any]]:
        """When legacy source tables are empty, fall back to existing ConsolidatedApp rows."""
        from app.models.consolidated_app import ConsolidatedApp

        existing_rows = ConsolidatedApp.query.limit(100).all()
        if not existing_rows:
            return []

        consolidated = [_model_to_dict_safe(r) for r in existing_rows]
        for r in consolidated:
            r.setdefault("_ai_predicted", [])
            r.setdefault("_ai_confidence", {})
            r.setdefault("_ai_model", None)
        logger.info(
            "Legacy source tables empty — using %d existing consolidated_apps rows for LLM analysis",
            len(consolidated),
        )
        return consolidated

    # Function: _annotate_with_llm
    @staticmethod
    def _annotate_with_llm(
        consolidated: List[Dict[str, Any]], ollama_available: bool, run_llm_annotation: bool
    ) -> None:
    ...
    # Function: _save_traceability_for_all_sources
    @classmethod
    def _save_traceability_for_all_sources(
        cls,
        consolidated: List[Dict[str, Any]],
        corent_records: List[Dict[str, Any]],
        cast_records: List[Dict[str, Any]],
        ...
        """Save PredictedFieldValue traceability rows for all three sources; return total rows saved."""
        from app.models.consolidated_app import ConsolidatedApp

        if not (ollama_available and correlation_result_id is not None):
            return 0
    ...
    # Function: _resolve_llm_analysis
    @classmethod
    def _resolve_llm_analysis(
        cls,
        ollama_available: bool,
        consolidated: List[Dict[str, Any]],
        stats: Dict[str, Any],
        ...
        correlation_result_id: Optional[int],
        timeout: int,
        llm_correlation_analysis_model: Any,
    ) -> Tuple[Dict[str, Any], Optional[int]]:
        ...
        return cls._execute_llm_step(
            consolidated, stats, predictions_summary,
            correlation_result_id, timeout, llm_correlation_analysis_model,
        )
    ...
    def run_full_pipeline(
        ...
        cls._mark_pre_enriched(corent_records + cast_records + industry_records, ollama_available)
        ...
        # ── Fallback: if legacy source tables are empty, load from existing ConsolidatedApp rows ──
        # The workspace Excel pipeline (ExcelPredictionService) stores data in
        # workspace_* tables, not the legacy cast_data/corent_data/industry_data tables.
        # When those legacy tables are empty, load from the consolidated_apps table which
        # was populated by a previous run, so the LLM analysis has real data to work with.
        if not consolidated:
            consolidated = cls._load_consolidated_fallback()

        # Optional per-app LLM annotation
        cls._annotate_with_llm(consolidated, ollama_available, run_llm_annotation)
        ...
        total_predictions = cls._save_traceability_for_all_sources(
            consolidated, corent_records, cast_records, industry_records,
            ollama_available, correlation_result_id,
        )
        ...
        _LLM_ANALYSIS_TIMEOUT = 220

        llm_analysis, llm_analysis_id = cls._resolve_llm_analysis(
            ollama_available, consolidated, stats, predictions_summary,
            correlation_result_id, _LLM_ANALYSIS_TIMEOUT, LLMCorrelationAnalysis,
        )
    ...
    # Function: save_predicted_field_values
    @staticmethod
    ...
    def get_consolidated_stats() -> Dict[str, Any]:
        ...
        correlation_match_type is copied verbatim from CorrelationService's per-match
        "match_type" field, which can be "direct", "server_name", "app_name_exact",
        "fuzzy", or "unmatched" (see correlation_service.py's _run_all_matching_phases).
        Only counting "direct"/"fuzzy"/"unmatched" here silently dropped apps matched
        via server_name/app_name_exact from every bucket, so direct+fuzzy+unmatched
        never summed to total — server_name/app_name_exact are both non-fuzzy exact
        matches, so they're folded into "direct_matches" alongside "direct".
        """
        ...
        direct = ConsolidatedApp.query.filter(
            ConsolidatedApp.correlation_match_type.in_(["direct", "server_name", "app_name_exact"])
        ).count()
        fuzzy = ConsolidatedApp.query.filter_by(correlation_match_type="fuzzy").count()
        unmatched = ConsolidatedApp.query.filter(
            ConsolidatedApp.correlation_match_type.in_(["unmatched", None])
        ).count()
