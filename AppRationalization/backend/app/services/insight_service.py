# ---------------------------------------------------------------------------
# Author: Vishnuu A
# Scope: Insight Service - Provides comprehensive dashboard data and analytics
# Date: 2026-05-13
# ---------------------------------------------------------------------------
"""
...
# Function: _build_cast_entry_collections
def _build_cast_entry_collections(cast_entries):
    arch_components = [
        {
            'app_id':             e.get('app_id', ''),
    ...
    return arch_components, int_deps, repo_mapping


# Function: _collect_dashboard_data
def _collect_dashboard_data():
    ...
    return industry_items, corent_items, cast_items, app_inv_items, latest_correlation


# Function: _build_insight_summary
def _build_insight_summary(
    corent_items, cast_items, app_inv_items, code_app_count, prog_lang_count,
    infrastructure_insights, risk_assessment, latest_correlation, industry_items,
):
...
# Function: _build_internal_deps_dict
def _build_internal_deps_dict(cast_items):
    internal_dependencies = {}
    for item in cast_items:
        if item.volume_external_dependencies:
            dep_count = (
                int(item.volume_external_dependencies)
                if isinstance(item.volume_external_dependencies, str)
                and item.volume_external_dependencies.isdigit()
                else item.volume_external_dependencies
            )
...
# Function: _build_repo_app_mapping
def _build_repo_app_mapping(app_inv_items):
    return [
        {
            'app_id':         item.app_id,
...
# Function: _derive_code_insights_from_correlation
def _derive_code_insights_from_correlation(latest_correlation, app_inv_items):
    """
    Build code_insights from the correlation layer when CAST data is empty.
    Returns (code_insights, code_app_count, prog_lang_count) or (None, None, None).
    """
    ...
    try:
        ...
        cast_entries = [
            c['cast_item']
            for c in corr_data.get('correlation_layer', [])
            if c.get('cast_item')
        ]
        ...
        arch_components, int_deps, repo_mapping = _build_cast_entry_collections(cast_entries)
        ...
        return code_insights, code_app_count, prog_lang_count
    except Exception:
...
class InsightService:
    ...
    # Function: get_dashboard_insights
    @staticmethod
    def get_dashboard_insights() -> Dict[str, Any]:
        ...
        industry_items, corent_items, cast_items, app_inv_items, latest_correlation = (
        ...
        if code_app_count == 0 and latest_correlation:
            derived_insights, derived_count, derived_lang_count = (
                _derive_code_insights_from_correlation(
                    latest_correlation, app_inv_items
                )
            )
            if derived_insights is not None:
                code_insights = derived_insights
                code_app_count = derived_count
                prog_lang_count = derived_lang_count
    ...
    def _analyze_code(cast_items: List[CASTData], app_inv_items: List[ApplicationInventory]) -> Dict[str, Any]:
        ...
        internal_dependencies = _build_internal_deps_dict(cast_items)

        # Build repo app mapping from app inventory
        repo_app_mapping = _build_repo_app_mapping(app_inv_items)
    ...
    # Function: _assess_infra_item_risk
    @staticmethod
    def _assess_infra_item_risk(item) -> Tuple[int, bool]:
        """Return (risk_score, is_cloud_ready) for one infra/IndustryData item."""
        risk_score = 0
        ...
        # Not cloud suitable
        cloud_suitability = getattr(item, 'cloud_suitability', None)
        is_cloud_ready = not (cloud_suitability and 'not' in str(cloud_suitability).lower())
        if not is_cloud_ready:
            risk_score += 1

        return risk_score, is_cloud_ready

    # Function: _count_cast_source_risk
    @staticmethod
    def _count_cast_source_risk(cast_items: List[CASTData]) -> int:
        count = 0
        for item in cast_items:
    ...
    # Function: _count_low_quality_app_inv
    @staticmethod
    def _count_low_quality_app_inv(app_inv_items: List[ApplicationInventory]) -> int:
        count = 0
        for item in app_inv_items:
    ...
    def _assess_risks(
        ...
        for item in items:
            risk_score, is_cloud_ready = InsightService._assess_infra_item_risk(item)
            if is_cloud_ready:
                cloud_ready_count += 1
            if risk_score >= 2:
                high_risk_count += 1
        ...
        high_risk_count += InsightService._count_cast_source_risk(cast_items)

        # Assess application inventory risks
        high_risk_count += InsightService._count_low_quality_app_inv(app_inv_items)
    ...
    # Function: _calculate_data_quality_score
    @staticmethod
    @staticmethod
    ...
    # Function: _estimate_maintenance_cost
    @staticmethod
    ...
    def get_dashboard_summary() -> Dict[str, Any]:
        ...
        return {
            ...
            'capabilities': 5,  # TODO: Query unique capabilities from consolidated data
            'duplicates': 0,  # app_id removed from CorentData during consolidation
            'maintenance_cost': InsightService._estimate_maintenance_cost(corent_items, app_inv_items),
            'scenarios': 3,  # TODO: Count actual transformation scenarios from business capability mapping
