# ---------------------------------------------------------------------------
# Author: Vishnuu A
# Scope: Service for extracting Corent data and populating CorentData table
# Date: 2026-05-17
# ---------------------------------------------------------------------------
...
class CorentDataService:
    ...
    @staticmethod
    def _extract_servers(infrastructure):
        try:
            if hasattr(infrastructure, 'servers'):
                return infrastructure.servers if infrastructure.servers else []
        except Exception as e:
            logger.warning(f"[CorentData] Could not access servers: {str(e)}")
        return []

    # Function: _upsert_corent_from_server
    @staticmethod
    def _upsert_corent_from_server(server) -> bool:
        """Create/update a CorentData record for one server; return True if a new record was created."""
    ...
    # Function: populate_from_infrastructure
    @staticmethod
    def populate_from_infrastructure(infrastructure):
        ...
        try:
            ...
            servers = CorentDataService._extract_servers(infrastructure)
            if not servers:
            ...
            for server in servers:
                try:
                    if CorentDataService._upsert_corent_from_server(server):
                        records_created += 1
                except Exception as e:
    ...
    # Function: _normalize_cell_value
    @staticmethod
    ...
    # Function: _populate_corent_row
    @staticmethod
    def _populate_corent_row(corent_record, row, dataframe_columns, column_mapping):
        for source_col, target_field in column_mapping.items():
            if source_col not in dataframe_columns:
                continue
            value = CorentDataService._normalize_cell_value(row.get(source_col))
    ...
    def populate_from_excel_file(file_path):
        ...
        try:
            ...
            for _, row in dataframe.iterrows():
                ...
                CorentDataService._populate_corent_row(corent_record, row, dataframe.columns, column_mapping)
