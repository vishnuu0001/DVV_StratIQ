# ---------------------------------------------------------------------------
# Author: Vishnuu A
# Scope: Standardization & Consolidation Analysis Service
# Date: 2026-01-06
# ---------------------------------------------------------------------------
"""
...
class StandardizationAnalysisService:
    ...
    # Function: _apply_workspace_fallback
    @staticmethod
    def _apply_workspace_fallback(corent, cast):
        """When legacy CorentData/CASTData tables are empty, fall back to the latest workspace run."""
        if corent and cast:
            return corent, cast
        try:
            ...
            if not latest_run:
                return corent, cast
            if not corent:
                corent = WorkspaceCorentRow.query.filter_by(run_id=latest_run.id).all()
            if not cast:
                cast = WorkspaceCastRow.query.filter_by(run_id=latest_run.id).all()
        except Exception as exc:
            logger.warning("Workspace fallback for standardization failed: %s", exc)
        return corent, cast

    # Function: _get_cached_or_fresh_llm_insights
    @staticmethod
    def _get_cached_or_fresh_llm_insights(analysis, now, ollama_service) -> dict:
        """Return cached LLM insights (< 2h old) or generate and cache a fresh set."""
        cached = StandardizationAnalysisService._llm_insights_cache
        if cached and (now - cached.get('_ts', 0)) < 7200:
            return {k: v for k, v in cached.items() if k != '_ts'}

        llm_insights = ollama_service.generate_standardization_insights(analysis)
        llm_insights['_ts'] = now
        StandardizationAnalysisService._llm_insights_cache = llm_insights
        return {k: v for k, v in llm_insights.items() if k != '_ts'}

    # Function: analyze_all_data
    @staticmethod
    def analyze_all_data() -> Dict[str, Any]:
        """Comprehensive analysis of CORENT and CAST data, enriched with LLM insights."""
        ...
        # Workspace fallback when legacy tables are empty
        corent, cast = StandardizationAnalysisService._apply_workspace_fallback(corent, cast)
        ...
        # Enrich with LLM insights if Ollama is available (cached for 2 hours)
        try:
            analysis['llm_insights'] = StandardizationAnalysisService._get_cached_or_fresh_llm_insights(
                analysis, _time.time(), OllamaService
            )
        except Exception as exc:
            logger.warning("Standardization LLM insights skipped: %s", exc)
            analysis['llm_insights'] = {"available": False, "error": str(exc)}

        return analysis
    
    # Function: _build_platform_host_stats
    @staticmethod
    def _build_platform_host_stats(corent_items: List) -> Dict[str, Any]:
        platform_hosts: Dict[str, Any] = {}
        for item in corent_items:
            if not item.platform_host:
                continue
    ...
    # Function: _group_ids_by_field
    @staticmethod
    def _group_ids_by_field(corent_items: List, field_getter) -> Dict[str, List[str]]:
        groups: Dict[str, List[str]] = {}
        for item in corent_items:
            key = field_getter(item)
            groups.setdefault(key, []).append(str(item.id))
        return groups

    # Function: _analyze_infrastructure
    @staticmethod
    def _analyze_infrastructure(corent_items: List) -> Dict[str, Any]:
        """Analyze infrastructure landscape"""

        # Server distribution
        platform_hosts = StandardizationAnalysisService._build_platform_host_stats(corent_items)

        # Environment distribution
        environments = StandardizationAnalysisService._group_ids_by_field(
            corent_items, lambda item: item.environment or 'Unknown'
        )

        # Cloud suitability
        cloud_suitability = StandardizationAnalysisService._group_ids_by_field(
            corent_items, lambda item: item.cloud_suitability or 'Unknown'
        )
    ...
    # Function: _identify_consolidation_opportunities
    @staticmethod
    ...
    # Function: _rec_cloud_migration
    @staticmethod
    def _rec_cloud_migration(corent_items):
        apps = [c for c in corent_items if c.cloud_suitability == 'High']
        if not apps:
            return None
        return {
            'priority': 'HIGH', 'category': 'Cloud Migration',
            'title': 'Migrate High-Readiness Applications to Cloud First',
            'value_proposition': f'{len(apps)} applications are already cloud-ready (High suitability)',
            'business_impact': 'Reduces on-premise infrastructure costs, improves scalability and elasticity',
            'apps_affected': len(apps),
            'estimated_savings': f'EUR {len(apps) * 15000}/year in infrastructure',
            'timeline': '6-12 months',
            'apps_list': [str(a.id) for a in apps][:5],
        }

    # Function: _rec_legacy_modernization
    @staticmethod
    def _rec_legacy_modernization(corent_items):
        apps = [c for c in corent_items if c.cloud_suitability in ['Low', 'Not Suitable']]
        if not apps:
            return None
        return {
            'priority': 'MEDIUM', 'category': 'Modernization',
            'title': 'Prioritize Modernization of Legacy Applications',
            'value_proposition': f'{len(apps)} applications have low/no cloud suitability - modernization needed',
            'business_impact': 'Enables future cloud migration, reduces technical debt, improves maintainability',
            'apps_affected': len(apps),
            'estimated_savings': f'EUR {len(apps) * 8000}/year in reduced maintenance',
            'timeline': '18-24 months',
            'apps_list': [str(a.id) for a in apps][:5],
        }

    # Function: _rec_database_consolidation
    @staticmethod
    def _rec_database_consolidation(corent_items):
        n = len([c for c in corent_items if c.db_engine and c.db_engine != 'Not Applicable'])
        if n <= 3:
            return None
        return {
            'priority': 'MEDIUM', 'category': 'Database Consolidation',
            'title': f'Consolidate {n} Database Technologies to 3-4 Standard Platforms',
            'value_proposition': 'Multiple database engines create complexity and support overhead',
            'business_impact': 'Standardization reduces operational complexity, improves automation potential',
            'apps_affected': n,
            'estimated_savings': f'EUR {n * 12000}/year in DBA support and licensing',
            'timeline': '12-18 months',
            'recommended_target': 'PostgreSQL, Oracle, MySQL - based on application requirements',
        }

    # Function: _rec_os_standardization
    @staticmethod
    def _rec_os_standardization(corent_items):
        n = len([c for c in corent_items if c.operating_system])
        if n <= 4:
            return None
        return {
            'priority': 'MEDIUM', 'category': 'OS Standardization',
            'title': f'Standardize Operating Systems ({n} current variants)',
            ...
            'apps_affected': n,
            'estimated_savings': f'EUR {n * 8000}/year in consolidated management',
            'timeline': '12-15 months',
            'recommended_target': 'RHEL/CentOS for Linux, Windows Server LTS for Windows workloads',
        }

    # Function: _rec_stability_consolidation
    @staticmethod
    def _rec_stability_consolidation(corent_items):
        unstable = [c for c in corent_items if c.application_stability in ['Unstable', 'Poor']]
        if len(unstable) <= 5:
            return None
        return {
            'priority': 'HIGH', 'category': 'Infrastructure Optimization',
            'title': f'Consolidate {len(unstable)} Unstable Applications',
            'value_proposition': f'{len(unstable)} applications have stability issues requiring migration',
            'business_impact': 'Improves system reliability, reduces downtime, enhances user experience',
            'apps_affected': len(unstable),
            'estimated_savings': f'EUR {len(unstable) * 5000}/year in reduced incident management',
            'timeline': '9-12 months',
            'apps_list': [str(a.id) for a in unstable][:5],
        }

    # Function: _rec_code_quality
    @staticmethod
    def _rec_code_quality(cast_items):
        with_source = [c for c in cast_items if c.source_code_availability == 'Yes']
        if len(with_source) <= 100:
            return None
        return {
            'priority': 'MEDIUM', 'category': 'Code Quality',
            'title': f'{len(with_source)} Applications Ready for Code Modernization',
            'value_proposition': f'{len(with_source)} applications have source code available - ready for refactoring',
            'business_impact': 'Reduces technical debt, improves code maintainability, enables faster feature delivery',
            'apps_affected': len(with_source),
            'estimated_savings': f'EUR {len(with_source) * 3000}/year in improved productivity',
            'timeline': 'Ongoing (phased approach)',
            'apps_list': [c.app_id for c in with_source][:5],
        }

    # Function: _rec_on_premise_consolidation
    @staticmethod
    def _rec_on_premise_consolidation(corent_items):
        n = len([c for c in corent_items if c.install_type in ['On Premise', 'On-Prem']])
        if n <= 50:
            return None
        return {
            'priority': 'MEDIUM', 'category': 'Infrastructure Consolidation',
            'title': f'Consolidate {n} On-Premise Applications',
            'value_proposition': f'{n} applications running on-premise represent ongoing capex/opex',
            'business_impact': 'Reduces physical infrastructure footprint, improves space utilization',
            'apps_affected': n,
            'estimated_savings': f'EUR {n * 2000}/year in datacenter costs',
            'timeline': '18-24 months',
        }

    # Function: _generate_business_recommendations
    @staticmethod
    def _generate_business_recommendations(corent_items: List, cast_items: List) -> List[Dict[str, Any]]:
        ...
        builders = [
            StandardizationAnalysisService._rec_cloud_migration(corent_items),
            StandardizationAnalysisService._rec_legacy_modernization(corent_items),
            StandardizationAnalysisService._rec_database_consolidation(corent_items),
            StandardizationAnalysisService._rec_os_standardization(corent_items),
            StandardizationAnalysisService._rec_stability_consolidation(corent_items),
            StandardizationAnalysisService._rec_code_quality(cast_items),
            StandardizationAnalysisService._rec_on_premise_consolidation(corent_items),
        ]
        recommendations = [r for r in builders if r is not None]
        return sorted(recommendations, key=lambda x: {'HIGH': 0, 'MEDIUM': 1, 'LOW': 2}.get(x['priority'], 3))
    
    # Function: _calculate_roi_impact
    @staticmethod
    ...
    # Function: _assess_consolidation_risks
    @staticmethod
