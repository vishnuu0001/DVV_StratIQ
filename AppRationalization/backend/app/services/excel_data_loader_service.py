# ---------------------------------------------------------------------------
# Author: Vishnuu A
# Scope: Excel Data Loader Service - Loads actual production data from Excel reports
# Date: 2025-08-27
# ---------------------------------------------------------------------------
"""
...
class ExcelDataLoaderService:
    ...
    def _normalize_cell_value(value):
        """Handle NaN and stray whitespace in a raw Excel cell value."""
        if pd.isna(value):
            return None
    ...
    # Function: _build_record_from_row
    @staticmethod
    def _build_record_from_row(row, df_columns, column_mapping) -> dict:
        record = {}
        for excel_col, db_field in column_mapping.items():
            if excel_col in df_columns:
                record[db_field] = ExcelDataLoaderService._normalize_cell_value(row[excel_col])
        return record

    # Function: _load_corent_data
    @staticmethod
    def _load_corent_data():
        ...
        try:
            ...
            for idx, row in df.iterrows():
                corent_record = ExcelDataLoaderService._build_record_from_row(row, df.columns, column_mapping)
    ...
    def _load_cast_data():
        ...
        try:
            ...
            count = 0
            for idx, row in df.iterrows():
                cast_record = ExcelDataLoaderService._build_record_from_row(row, df.columns, column_mapping)
