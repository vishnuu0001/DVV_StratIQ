# ---------------------------------------------------------------------------
# Author: Vishnuu A
# Scope: Orchestrates the "Correlate Files & Start Analysis" workspace pipeline:
# Date: 2025-09-12
# ---------------------------------------------------------------------------
"""
...
# ---------------------------------------------------------------------------
# Pipeline helpers (used by ExcelPredictionService.run_workspace_pipeline)
# ---------------------------------------------------------------------------

# Function: _get_cached_run_result
def _get_cached_run_result(app, excel_hash):
    """Return the cached run result dict if Excel files are unchanged, else None."""
...
# Function: _run_parallel_source_predictions
def _run_parallel_source_predictions(app, run_id, predict_fn, sources_cfg):
    """Run LLM null-field predictions for all sources in parallel; return all updates."""
    all_updates: List[Dict[str, Any]] = []

    # Function: _run_source
    def _run_source(src_label, model_cls, col_map):
        try:
            updates = predict_fn(
                model_cls, run_id, list(col_map.values()), src_label, app,
            )
            with app.app_context():
                db.session.commit()
            return updates
            ...
            logger.error(
                ...
                src_label, exc, traceback.format_exc(),
            )
            return []

    with ThreadPoolExecutor(max_workers=3) as pool:
        futures = {
            pool.submit(_run_source, sl, mc, cm): sl
            for sl, mc, cm in sources_cfg
        }
...
class ExcelPredictionService:
    ...
    # Function: _compute_excel_hash
    @staticmethod
    def _compute_excel_hash() -> str:
        """
        Return a short SHA-256 fingerprint of the three source Excel files
    ...
    # Function: _apply_heuristic_prefill
    @staticmethod
    def _apply_heuristic_prefill(
        rows: List[Any],
        ...
        """Apply deterministic heuristic fills before invoking LLM."""
        now = datetime.utcnow()
        model_used_heuristic = "heuristic"
        for row_obj in rows:
            rec = {f: getattr(row_obj, f, None) for f in field_names if f not in skip_fields}
    ...
    # Function: _collect_llm_candidates
    @staticmethod
    def _collect_llm_candidates(
        ...
        """Return rows that still contain null fields after heuristic pass."""
    ...
    # Function: _apply_llm_predictions
    @staticmethod
    def _apply_llm_predictions(
        ...
        """Run batch LLM prediction and apply updates in-place to rows."""
        if not candidates:
            return

        all_records = [rec for _, rec in candidates]
        batch_results = OllamaService.predict_missing_fields_batch(
            all_records, source=llm_source, batch_size=15
        )
        model_used = OllamaService.get_selected_model()

        for (row_obj, original_rec), (predictions, _pred_cols, confidence_map) in zip(candidates, batch_results):
            if not predictions:
                continue
    ...
    # Function: _persist_row_fill_metadata
    @staticmethod
    def _persist_row_fill_metadata(rows: List[Any], row_fills: Dict[int, List[str]]) -> None:
        """Persist updated_rows + ai_predicted_columns for touched rows."""
        for row_obj in rows:
    ...
    # Function: _predict_and_update_db
    @staticmethod
    def _predict_and_update_db(
        ...
        with app.app_context():
            ...
            ExcelPredictionService._apply_heuristic_prefill(
                rows=rows,
            ...
            candidates = ExcelPredictionService._collect_llm_candidates(
                rows=rows,
                field_names=field_names,
                skip_fields=_skip,
            )
            ...
            if not candidates:
                ...
                all_records = [rec for _, rec in candidates]
                try:
                    ExcelPredictionService._apply_llm_predictions(
            ...
            ExcelPredictionService._persist_row_fill_metadata(rows=rows, row_fills=row_fills)
    ...
    # Function: _build_canonical_app_names
    @staticmethod
    def _build_canonical_app_names(corent_rows, biz_rows, cast_rows) -> Dict[str, str]:
        """Build the canonical app_name per app_id. Authority order: CAST > Business > CORENT."""
    ...
    # Function: _sync_single_row_app_name
    @staticmethod
    def _sync_single_row_app_name(row, canonical, run_id, source_label, now) -> Optional[Dict[str, Any]]:
        """Apply the canonical app_name to one row if it's missing one; return the traceability entry."""
    ...
    # Function: _sync_app_names_across_sources
    @staticmethod
    def _sync_app_names_across_sources(run_id: int) -> List[Dict[str, Any]]:
        ...
        canonical = ExcelPredictionService._build_canonical_app_names(corent_rows, biz_rows, cast_rows)
        if not canonical:
            return []
        ...
        for row_list, source_label in [
            ...
            for row in row_list:
                update = ExcelPredictionService._sync_single_row_app_name(row, canonical, run_id, source_label, now)
                if update:
    ...
    # Function: _sync_cast_to_corent_architecture
    @staticmethod
    def _sync_cast_to_corent_architecture(cast_by_app_id, corent_by_app_id, run_id, now) -> List[Dict[str, Any]]:
        """CAST application_architecture → CORENT architecture_type (only when CORENT's is null)."""
        sync_updates: List[Dict[str, Any]] = []
        for aid_upper, cast_row in cast_by_app_id.items():
            ...
            if not corent_row or corent_row.architecture_type:
                continue
    ...
    # Function: _sync_corent_to_cast_architecture
    @staticmethod
    def _sync_corent_to_cast_architecture(corent_by_app_id, cast_by_app_id, run_id, now) -> List[Dict[str, Any]]:
        """CORENT architecture_type → CAST application_architecture (only when CAST's is null)."""
        sync_updates: List[Dict[str, Any]] = []
        for aid_upper, corent_row in corent_by_app_id.items():
            ...
            if not cast_row or cast_row.application_architecture:
                continue
    ...
    # Function: _sync_architecture_across_sources
    @staticmethod
    def _sync_architecture_across_sources(run_id: int) -> List[Dict[str, Any]]:
        ...
        sync_updates = ExcelPredictionService._sync_cast_to_corent_architecture(
            cast_by_app_id, corent_by_app_id, run_id, now
        )
        sync_updates += ExcelPredictionService._sync_corent_to_cast_architecture(
            corent_by_app_id, cast_by_app_id, run_id, now
        )
    ...
    # Function: _match_corent_for_cast_row
    @staticmethod
    def _match_corent_for_cast_row(cast_row, corent_by_app_id, corent_by_server, corent_by_app_name):
        """3-level CORENT match for a CAST row; returns (corent_row, confidence)."""
        app_id = cast_row.app_id
        ...
        if corent_row is None and cast_row.app_name:
            ...
            if corent_row:
                corent_conf = 0.70
        return corent_row, corent_conf

    # Function: _match_biz_for_cast_row
    @staticmethod
    def _match_biz_for_cast_row(cast_row, biz_by_app_id, biz_by_app_name):
        """2-level Business match for a CAST row; returns (biz_row, confidence)."""
        app_id = cast_row.app_id
        biz_row = None
        biz_conf = 0.0
        if app_id:
        ...
        if biz_row is None and cast_row.app_name:
            ...
            if biz_row:
                biz_conf = 0.75
        return biz_row, biz_conf

    # Function: _determine_match_type_and_confidence
    @staticmethod
    def _determine_match_type_and_confidence(corent_row, corent_conf, biz_row, biz_conf):
        """Return (match_type, match_confidence) from corent/biz match results."""
        if corent_row and biz_row:
            return "three_way", round((corent_conf + biz_conf) / 2, 2)
        if biz_row:
            return "cast_biz", round(biz_conf * 0.85, 2)
        if corent_row:
            return "cast_corent", round(corent_conf * 0.80, 2)
        return "unmatched", 0.0

    # Function: _build_app_name_index
    @staticmethod
    def _build_app_name_index(rows) -> Dict[str, Any]:
        """Build a normalized-app_name → row lookup (first row wins per normalized name)."""
        index: Dict[str, Any] = {}
        for r in rows:
            norm = _normalize_app_name(r.app_name)
            if norm:
                index.setdefault(norm, r)
        return index

    # Function: _build_correlation_row
    @staticmethod
    def _build_correlation_row(run_id, cast_row, corent_row, biz_row, match_type, match_confidence) -> "WorkspaceCorrelation":
        ...
        return WorkspaceCorrelation(
            ...
            cast_server_name=cast_row.server_name,
    ...
    def _correlate_from_db(run_id: int) -> Tuple[int, float]:
        ...
        corent_by_app_name = ExcelPredictionService._build_app_name_index(corent_rows)
        biz_by_app_name = ExcelPredictionService._build_app_name_index(biz_rows)
        ...
        for cast_row in cast_rows:
            corent_row, corent_conf = ExcelPredictionService._match_corent_for_cast_row(
                cast_row, corent_by_app_id, corent_by_server, corent_by_app_name
            )
            biz_row, biz_conf = ExcelPredictionService._match_biz_for_cast_row(
                cast_row, biz_by_app_id, biz_by_app_name
            )
            match_type, match_confidence = ExcelPredictionService._determine_match_type_and_confidence(
                corent_row, corent_conf, biz_row, biz_conf
            )

            if match_type != "unmatched":
                matched += 1

            correlations.append(ExcelPredictionService._build_correlation_row(
                run_id, cast_row, corent_row, biz_row, match_type, match_confidence
            ))
    ...
    # ------------------------------------------------------------------ #
    # Step 2 helper — bulk-load all source rows
    # ------------------------------------------------------------------ #

    # Function: _load_source_rows_to_db
    @classmethod
    def _load_source_rows_to_db(cls, app, cast_df, corent_df, biz_df, run_id):
        """Bulk-insert raw Excel rows into workspace DB tables for this run."""
        with app.app_context():
        ...
        return cast_count, corent_count, biz_count

    # Function: _finalize_run_record
    @classmethod
    def _finalize_run_record(cls, app, run_id, all_updates, cast_count, corent_count, biz_count):
        """Save traceability rows, correlate from DB, update WorkspaceRun to done; return result dict."""
        with app.app_context():
    ...
    # Function: run_workspace_pipeline
    @classmethod
    def run_workspace_pipeline(cls, app: Any) -> Dict[str, Any]:
        ...
        cached = _get_cached_run_result(app, excel_hash)
        if cached is not None:
            return cached

        with app.app_context():
            # Enable WAL mode for SQLite to allow concurrent readers during
            # multi-threaded prediction writes (no-op for PostgreSQL).
            try:
                db.session.execute(db.text("PRAGMA journal_mode=WAL"))
                db.session.execute(db.text("PRAGMA busy_timeout=30000"))
                db.session.commit()
            except Exception:
                pass  # Non-SQLite databases don't support PRAGMA
        ...
        try:
            ...
            # Fail loudly instead of silently completing with 0 rows. _read_excel()
            # deliberately swallows read errors (FileNotFoundError, bad sheet name,
            # etc.) and returns an empty DataFrame so one bad file doesn't take down
            ...
            _missing = [
                str(path) for path, df in (
                    (_CAST_SRC, cast_df), (_CORENT_SRC, corent_df), (_BIZ_SRC, biz_df),
                ) if df.empty
            ]
            if _missing:
                raise FileNotFoundError(
                    "Workspace pipeline source file(s) missing or unreadable: "
                    + "; ".join(_missing)
                )

            # ── Step 2: load raw rows to DB ───────────────────────────────
            cast_count, corent_count, biz_count = cls._load_source_rows_to_db(
                app, cast_df, corent_df, biz_df, run_id
            )
            ...
            all_updates = _run_parallel_source_predictions(
                app, run_id, cls._predict_and_update_db, sources_cfg,
            )
            ...
            result = cls._finalize_run_record(
                app, run_id, all_updates, cast_count, corent_count, biz_count
            )
            logger.info(
                "workspace_pipeline run_id=%d done: CAST=%d CORENT=%d Biz=%d "
                "matched=%d (%.1f%%) ai_cells=%d",
                run_id, cast_count, corent_count, biz_count,
                result["matched_count"], result["match_pct"], len(all_updates),
            )
            return result
