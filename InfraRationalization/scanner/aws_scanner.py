...
# Function: _ec2_collect_interfaces
def _ec2_collect_interfaces(inst: dict) -> tuple:
    ...
    primary_ip = private_ip or public_ip or inst.get("InstanceId", "")
    return ifaces, primary_ip


# Function: _ec2_collect_disks
def _ec2_collect_disks(inst: dict, ec2_client) -> tuple:
    disks: list[DiskInfo] = []
    total_gb = 0.0
...
def _ec2_to_server(inst: dict, region: str, ec2_client) -> DiscoveredServer:
    ...
    ifaces, primary_ip = _ec2_collect_interfaces(inst)
    disks, total_gb = _ec2_collect_disks(inst, ec2_client)
...
# Function: _elasticache_to_server
