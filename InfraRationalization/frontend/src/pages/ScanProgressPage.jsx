// ---------------------------------------------------------------------------
// Author: Vishnuu A
// Scope: InfraRationalization — frontend/src/pages (ScanProgressPage.jsx)
// Date: 2026-06-16
// ---------------------------------------------------------------------------
    ...
    es.onmessage = (event) => {
      try {
        ...
        if (data.progress >= 100 || data.status === 'completed' || data.status === 'failed') {
          es.close()
          // SSE events don't always include status — poll once for the real final state
          setTimeout(() => {
            getScanJob(scanId).then(j => {
              setStatus(j.status)
              setProgress(j.progress || 100)
              setMessage(j.progress_message || data.message)
