...
def _timeout_seconds() -> float:
    return max(2.0, min(10.0, float(cfg.OLLAMA_TIMEOUT_SECONDS) / 12.0))
...
def get_ollama_runtime_status(preferred_model: Optional[str] = None) -> dict[str, Any]:
    ...
    try:
        with httpx.Client(timeout=_timeout_seconds()) as client:
            ps_resp = client.get(f"{cfg.OLLAMA_BASE_URL.rstrip('/')}/api/ps")
...
def _warm_ollama_model(model_name: str) -> None:
    ...
    with httpx.Client(timeout=max(8.0, float(cfg.OLLAMA_TIMEOUT_SECONDS))) as client:
        client.post(
            ...
            json={
                ...
                "options": {
                    ...
                    "num_ctx": min(1024, cfg.OLLAMA_NUM_CTX),
                },
            },
        ).raise_for_status()
...
def assert_ollama_gpu_available(model_name: Optional[str] = None) -> None:
    ...
    target_model = model_name or cfg.OLLAMA_MODEL
...
def get_llm(provider: str) -> Any:
    ...
    if provider == "openai":
        ...
        logger.info("Using OpenAI LLM: %s", cfg.OPENAI_MODEL)
        return ChatOpenAI(
