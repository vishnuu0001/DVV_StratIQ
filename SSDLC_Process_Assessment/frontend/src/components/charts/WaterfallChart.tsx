// ---------------------------------------------------------------------------
// Author: Vishnuu A
// Scope: SSDLC_Process_Assessment — frontend/src/components/charts (WaterfallChart.tsx)
// Date: 2026-06-09
// ---------------------------------------------------------------------------
import {
  BarChart,
...
export default function WaterfallChart({ gross: _gross, transitionCost: _tc, net: _net }: WaterfallProps) {
  const gross = typeof _gross === 'string' ? parseFloat(_gross) || 0 : _gross
  const transitionCost = typeof _tc === 'string' ? parseFloat(_tc) || 0 : _tc
  const net = typeof _net === 'string' ? parseFloat(_net) || 0 : _net
