// ---------------------------------------------------------------------------
// Author: Vishnuu A
// Scope: SSDLC_Process_Assessment — frontend/src/pages (TransformationCapacityPage.tsx)
// Date: 2026-03-31
// ---------------------------------------------------------------------------
...
type ChartAllocation = ReinvestmentAllocation & {
  allocation_pct: number
  funding: number
}

// Function: allocationPct
function allocationPct(item: ReinvestmentAllocation & { allocation_pct?: number | string }): number {
  return toNum(item.allocation_pct ?? item.pct)
}
...
function ScenarioCard({ scenario, isRecommended }: { scenario: CapacityScenario; isRecommended: boolean }) {
  ...
  const optimizationRate = toNum(scenario.optimization_rate)
  return (
    <div className={`relative ${style.bg} border ${style.border} rounded-xl p-6`}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`text-xs font-medium ${style.accent}`}>
          {formatPctDirect(optimizationRate * 100)} opt. rate
        </span>
      </div>
        ...
        <div>
          <div className="h-1.5 bg-navy-700 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(optimizationRate * 100 * 4, 100)}%` }}
            />
          </div>
        </div>
      </div>
...
function AllocationTooltip({ active, payload }: { active?: boolean; payload?: TooltipPayload[] }) {
  ...
  return (
    <div className="bg-navy-700 border border-navy-600 rounded-lg p-3 text-xs shadow-xl">
      ...
      <div className="text-slate-500">{formatPctDirect(item.payload.allocation_pct)} of pool</div>
    </div>
  )
}
...
export default function TransformationCapacityPage() {
  ...
  const allocations: ChartAllocation[] = (data?.reinvestment_allocations ?? []).map((item) => ({
    ...item,
    funding: toNum(item.funding),
    allocation_pct: allocationPct(item),
  }))
              ...
              <tbody className="divide-y divide-navy-700/50">
                {allocations.map((item, i) => (
                  <tr key={item.area} className="hover:bg-navy-700/20">
                    ...
                    <td className="py-2.5 text-right text-slate-400">{formatPctDirect(item.allocation_pct)}</td>
