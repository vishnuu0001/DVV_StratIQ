// ---------------------------------------------------------------------------
// Author: Vishnuu A
// Scope: SSDLC_Process_Assessment — frontend/src/pages (TechMGrowthPage.tsx)
// Date: 2026-04-19
// ---------------------------------------------------------------------------
    ...
    <div className={`${style.bg} border ${style.border} rounded-xl p-6 flex flex-col gap-4`}>
      ...
      <div className="grid grid-cols-2 gap-3 text-xs">
        ...
        <div>
          ...
          <div className={`font-medium ${style.accent}`}>{formatPctDirect(toNum(scenario.growth_pct) * 100)}</div>
        </div>
        <div>
...
export default function TechMGrowthPage() {
  ...
  const opportunityChartData = (data?.opportunity_areas ?? []).map((o: OpportunityArea) => ({
    area: o.area,
    'Low Case': toNum(o.low),
    'Expected Case': toNum(o.expected),
    'High Case': toNum(o.high),
  }))
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="max-w-xs">
        <KpiCard
          ...
          value={toNum(data?.current_techm_spend ?? 0)}
          format="currency"
          accent="blue"
          loading={isLoading}
