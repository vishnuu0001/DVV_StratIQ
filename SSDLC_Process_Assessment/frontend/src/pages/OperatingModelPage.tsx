// ---------------------------------------------------------------------------
// Author: Vishnuu A
// Scope: SSDLC_Process_Assessment — frontend/src/pages (OperatingModelPage.tsx)
// Date: 2026-06-12
// ---------------------------------------------------------------------------
...
function listOrDefault<T>(value: unknown, fallback: T[]): T[] {
  return Array.isArray(value) ? value as T[] : fallback
}

// Function: normalizeOperatingModel
function normalizeOperatingModel(data?: Partial<OperatingModelResponse>): OperatingModelResponse {
  const source = data ?? {}
  return {
    participants: listOrDefault<OperatingModelParticipant>(source.participants, STATIC_DATA.participants).map((item, index) => ({
      ...STATIC_DATA.participants[index],
      ...item,
      responsibilities: listOrDefault<string>(item?.responsibilities, STATIC_DATA.participants[index]?.responsibilities ?? []),
    })),
    roadmap: listOrDefault<OperatingModelRoadmapItem>(source.roadmap, STATIC_DATA.roadmap).map((item, index) => ({
      ...STATIC_DATA.roadmap[index],
      ...item,
      activities: listOrDefault<string>(item?.activities, STATIC_DATA.roadmap[index]?.activities ?? []),
    })),
    raci: listOrDefault<RaciEntry>(source.raci, STATIC_DATA.raci),
    priority_areas: listOrDefault<{ area: string; description: string; impact: string }>(
      source.priority_areas,
      STATIC_DATA.priority_areas,
    ),
  }
}
...
export default function OperatingModelPage() {
  ...
  const data = normalizeOperatingModel(apiData)
