...
class PlaywrightEmitter:
    ...
    async def generate(
        ...
        body = deterministic_script_body("playwright", test_case, requirement)
        ...
        generated = (
            ...
            f"test('@{requirement.req_id} @{test_case.tc_id} {test_case.title}', async ({{ page }}) => {{\n"
            f"{body}\n"
            "});\n\n"
            "// <traceforge:custom>\n"
        ...
        return code, f"tests/e2e/{slug}.spec.ts", None
